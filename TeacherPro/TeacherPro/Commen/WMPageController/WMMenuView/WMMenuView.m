...
- (void)addItems {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        item.selectedFont = systemMediumFontSize(16)//[UIFont fontWithName:self.selectfontName size:item.selectedSize];
        ...
        btn.tag = WMMenuItemTagOffset + i + 1000;
        [btn addTarget:self action:@selector(btnAction:) forControlEvents:UIControlEventTouchUpInside];
        [self.scrollView addSubview:btn];

        [btn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.leading.bottom.trailing.equalTo(item);
        }];
    }
}
