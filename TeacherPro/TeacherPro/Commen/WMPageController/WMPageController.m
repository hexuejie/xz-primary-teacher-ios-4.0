...
- (void)wm_calculateSize {
    if ([self.dataSource respondsToSelector:@selector(pageController:preferredFrameForMenuView:)]) {
        ...
        _menuViewFrame = CGRectMake(0, originY, self.view.frame.size.width, 44.0f);
    }
...
- (CGFloat)wm_calculateItemWithAtIndex:(NSInteger)index {
    ...
    UIFont *titleFont = calucaleFontName ? [UIFont fontWithName:calucaleFontName size:self.titleSizeSelected] : [UIFont systemFontOfSize:self.titleSizeSelected weight:UIFontWeightMedium];
...
- (void)menuView:(WMMenuView *)menu didSelesctedIndex:(NSInteger)index currentIndex:(NSInteger)currentIndex {
    ...
    _selectIndex = (int)index;
    _startDragging = NO;    CGPoint targetP = CGPointMake(_contentViewFrame.size.width * index, 0);
