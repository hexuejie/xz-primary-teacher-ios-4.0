...
- (void)setupMainView
{
    ...
    [mainView registerClass:[SDCollectionViewCell class] forCellWithReuseIdentifier:ID];
     [mainView registerNib:[UINib nibWithNibName:NSStringFromClass([SDCollectionPreCollectionViewCell class]) bundle:nil] forCellWithReuseIdentifier:@"SDCollectionPreCollectionViewCell"];
    
//    998  SDCollectionPreCollectionViewCell
...
{// 备注 绘本单个cell
    if (self.tag == 998) {
       NSDictionary *tempDic = self.dataArray[indexPath.item];
        ...
        SDCollectionPreCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"SDCollectionPreCollectionViewCell" forIndexPath:indexPath];
        cell.dataDic = tempDic;
        cell.isChinese = self.isChinese;
        
        cell.backgroundColor = project_background_gray;
        cell.contentView.backgroundColor = project_background_gray;
        [cell.contentView setCornerRadius:4 withShadow:YES withOpacity:10];
     
        NSString *imagePath = tempDic[@"image"];
        
        if (!self.onlyDisplayText && [imagePath isKindOfClass:[NSString class]]) {
            if ([imagePath hasPrefix:@"http"]) {
                [cell.advImageView sd_setImageWithURL:[NSURL URLWithString:imagePath] placeholderImage:self.placeholderImage  ];
            } else {
                UIImage *image = [UIImage imageNamed:imagePath];
                if (!image) {
                    [UIImage imageWithContentsOfFile:imagePath];
                }
                cell.advImageView.image = image;
            }
        } else if (!self.onlyDisplayText && [imagePath isKindOfClass:[UIImage class]]) {
            cell.advImageView.image = (UIImage *)imagePath;
        }
        ...
        if (!cell.hasConfigured) {
            ...
            cell.titleLabelTextAlignment = NSTextAlignmentCenter;
            cell.titleLabelTextColor = HexRGB(0x8A8F99);
            cell.titleLabelTextFont = [UIFont systemFontOfSize:17];
            cell.hasConfigured = YES;
            cell.advImageView.contentMode = UIViewContentModeScaleAspectFill;
