...
- (instancetype)initWithFrame:(CGRect)frame
                ...
                pathFillColor:(UIColor *)pathFillColor
                  ...
                  strokeWidth:(CGFloat)strokeWidth textStyle:(ZZCircleProgressTextStyle)style{
    if (self = [super initWithFrame:frame]) {
        [self initialization];
        if (pathBackColor) {
...
- (void)initialization {
    ...
    _pathBackColor = [UIColor clearColor];
...
- (CAShapeLayer *)progressLayer {
    if (!_progressLayer) {
        ...
        [_gradientLayer1 setColors:[NSArray arrayWithObjects:(id)[HexRGB(0x4ABCFF) CGColor],(id)[HexRGB(0x2487FF) CGColor], nil]];
//        [_gradientLayer1 setLocations:@[@0.5,@0.9,@1 ]];
        [_gradientLayer1 setStartPoint:CGPointMake(0.75, 0.25)];
        [_gradientLayer1 setEndPoint:CGPointMake(0.25, 0.75)];
...
- (UIImageView *)pointImage {
    if (!_pointImage) {
        ...
        _pointImage.layer.cornerRadius = 3.0;
        _pointImage.layer.masksToBounds = YES;
        _pointImage.frame = CGRectMake(0, 0, 6, 6);
        //定位起点位置
...
- (ZZCountingLabel *)progressLabel {
    if (!_progressLabel) {
        ...
        if (self.textStyle == ZZCircleProgressTextStyle_normal) {
             _progressLabel.text = @"0%";
        }else if (self.textStyle == ZZCircleProgressTextStyle_custom){
            if (self.progressTextAtr) {
                _progressLabel.attributedText = self.progressTextAtr;
            }else{
                _progressLabel.text = self.progressText;
                
            }
        }
       
        _progressLabel.numberOfLines = 2;
        _progressLabel.frame = CGRectMake(0, -1, ZZCircleSelfWidth, ZZCircleSelfHeight);
    }
...
- (void)setStartAngle:(CGFloat)startAngle {
    if (_startAngle != ZZCircleDegreeToRadian(startAngle)) {
        ...
        if (_progressLayer) {
            ...
            _gradientLayer1.mask = _progressLayer;
