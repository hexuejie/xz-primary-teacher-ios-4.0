...
#define  LX_iPhoneX                 ([UIScreen instancesRespondToSelector:@selector(currentMode)] ? CGSizeEqualToSize(CGSizeMake(1125, 2436), [[UIScreen mainScreen] currentMode].size) : NO)
#define  LX_HOME_INDICATOR_HEIGHT (LX_iPhoneX ? 34.f : 0)
...
//                        btnImage = [btnImage transToGrayImage];
    ...
    if (shareButtonImagesNameArray && shareButtonImagesNameArray.count > 0) {
        ...
        if (self.isHadTitle  == YES) {
            self.LXActivityHeight = self.LXActivityHeight + totalColumns*(SHAREBUTTON_INTERVAL_HEIGHT+SHAREBUTTON_HEIGHT)  +LX_HOME_INDICATOR_HEIGHT;
        }
        else{
            self.LXActivityHeight = SHAREBUTTON_INTERVAL_HEIGHT + totalColumns*(SHAREBUTTON_INTERVAL_HEIGHT+SHAREBUTTON_HEIGHT) + LX_HOME_INDICATOR_HEIGHT;
        }
    }
...
//            [cancelButton setFrame:CGRectMake(cancelButton.frame.origin.x, self.LXActivityHeight, cancelButton.frame.size.width, cancelButton.frame.size.height)];
            [bottomView setFrame: CGRectMake(cancelButton.frame.origin.x, self.LXActivityHeight- LX_HOME_INDICATOR_HEIGHT, cancelButton.frame.size.width, cancelButton.frame.size.height)];
