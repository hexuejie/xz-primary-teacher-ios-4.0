 ...
 *  开启任务下载资源
 */
- (void)download:(NSString *)url progress:(void (^)(NSInteger, NSInteger, CGFloat))progressBlock state:(void (^)(DownloadState state, NSString *url))stateBlock
{
    if (!url) return;
    if ([self isCompletion:url]) {
        stateBlock(DownloadStateCompleted,url);
        NSLog(@"----该资源已下载完成");
        return;
    }
...
- (void)start:(NSString *)url
{
    ...
    [self getSessionModel:task.taskIdentifier].stateBlock(DownloadStateStart,url);
}
...
- (void)pause:(NSString *)url
{
    ...
    [self getSessionModel:task.taskIdentifier].stateBlock(DownloadStateSuspended,url);
}
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error
{
    ...
    if ([self isCompletion:sessionModel.url]) {
        // 下载完成
        sessionModel.stateBlock(DownloadStateCompleted,sessionModel.url);
    } else if (error){
        // 下载失败
        sessionModel.stateBlock(DownloadStateFailed,sessionModel.url);
    }
...
- (NSString *)getFileAudioPath:(NSString *)url{

    NSString * audioPath = [HSCachesDirectory stringByAppendingPathComponent:HSFileName(url)];
    return audioPath;
}
@end
