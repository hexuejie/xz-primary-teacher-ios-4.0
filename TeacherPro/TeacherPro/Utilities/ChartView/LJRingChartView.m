...
@interface LJRingChartView () <CAAnimationDelegate>
...
@implementation LJRingChartView
...
- (void)drawRect:(CGRect)rect {
    ...
    for (LJChartItem *item in self.chartItems) {
    ...
    for (NSInteger i = 0; i < self.chartItems.count; i++) {
        ...
        LJChartItem *item = self.chartItems[i];
...
- (CAShapeLayer *)drawRingWithArcCenter:(CGPoint)centerPoint radius:(CGFloat)radius startAngle:(CGFloat)startAngle endAngle:(CGFloat)endAngle lineWidth:(CGFloat)lineWidth item:(LJChartItem *)item {
...
- (UIView *)drawDotWithOuterRingRadius:(CGFloat)outerRingRadius radianCenter:(CGFloat)radianCenter centerPoint:(CGPoint)centerPoint item:(LJChartItem *)item {
